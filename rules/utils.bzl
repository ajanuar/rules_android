...
def get_android_toolchain(ctx):
    return ctx.toolchains["@build_bazel_rules_android//toolchains/android:toolchain_type"]
...
def get_android_sdk(ctx):
    if hasattr(ctx.fragments.android, "incompatible_use_toolchain_resolution") and ctx.fragments.android.incompatible_use_toolchain_resolution:
        return ctx.toolchains["@build_bazel_rules_android//toolchains/android_sdk:toolchain_type"].android_sdk_info
    else:
